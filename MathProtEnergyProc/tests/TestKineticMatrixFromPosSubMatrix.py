...
#Модульные тесты
class TestKineticMatrixFromPosSubMatrix(unittest.TestCase):
    def setUp(self):
    ...
    #Модульные тесты
    def testKineticMatrixFromPosSubMatrix1(self):
        #Исходные данные
        ...
        etKineticMatrix = np.dot(np.dot(etSubMatrixBalance,etPosSubMatrix),etSubMatrixBalance.transpose())
        
        #Вызываем функцию
        KineticMatrix = KineticMatrixFromPosSubMatrix(posSubMatrixes,#Положительные определенные составляющие атрицы
    ...
    def testKineticMatrixFromPosSubMatrix2(self):
        #Исходные данные
        ...
        err = np.max(np.abs(KineticMatrix - etKineticMatrix))
        self.assertAlmostEqual(err, 0.0, 9)
    def testKineticMatrixFromPosSubMatrix3(self):
