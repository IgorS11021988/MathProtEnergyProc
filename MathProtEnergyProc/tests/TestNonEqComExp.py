...
class TestNonEqSystemComExp(unittest.TestCase):
    ...
    def testNonEqDyns1(self):
        ...
        def CompressFunction(dyn, index):
            return (np.sum(dyn[0]*dyn[1]),np.sum(dyn[0]*dyn[2]),index)
    ...
    def testNonEqDyns5(self):
        ...
        def CompressFunction(dyn, index):
            #Выводим результат
            return (np.sum(dyn[0]*dyn[1]*dyn[3]),np.sum(dyn[0]*dyn[2] + dyn[3]),np.sum(dyn[0]*dyn[1]),index)
    ...
    def testNonEqDyns9(self):
        ...
        def CompressFunction(dyn, index):
            #Выводим результат
            return (np.sum(dyn[0]*dyn[1] + dyn[3]),np.sum(dyn[0]*dyn[2]*dyn[3]),np.sum(dyn[0]*dyn[2]),index)
