...
from MathProtEnergyProc import CountDynamicsQ, ModelLearningQ
...
#Модульные тесты
class TestNonEqSystemComExpModelLearningQ(unittest.TestCase):
    def setUp(self):
    ...
    def testNonEqDyn1(self):
        ...
        cDynLearn = ModelLearningQ(funSysPar,cDyn)
        ...
        deltaChar3 = np.max(np.abs(funSysPar.Par[2] - reducedTemp[0]))
        self.assertAlmostEqual(deltaChar3, 0.0, 9)
        deltaChar4 = np.max(np.abs(funSysPar.Par[3] - systemParameters[0]))
        self.assertAlmostEqual(deltaChar4, 0.0, 9)
    def testNonEqDyn2(self):
        ...
        def countSystem2(t,allStParameters,systemParameters):
            ...
            (chemPot,potBet,Temp,Betas,Aff,AffHeat,
             ...
             vx,vUPows,invC,powH,vT,heatStreams,Streams) = CountSystemQ2(_stateCoordinates,_reducedTemp,_systemParameters,
                                                                         ...
                                                                         Adiff1_2,Adiff2_1,ADiffHeat2,AHeatDiff2,invC1,H1_1,
                                                                         H2_7,Tokr,AHeatOkr,ExtQEnPow1,AChem1_4_4,mu2_9,
                                                                         AChem2_3_3,AChem2_1_2,AChem2_2_1,Adiff3_3,ADiffHeat3,
                                                                         AHeatDiff3,xExt1_6,muBet1_3,muBet1_5)
        ...
        self.assertAlmostEqual(deltaChar2, 0.0, 9)
        deltaChar3 = np.max(np.abs(funSysPar.Par[2] - reducedTemp[1]))
        self.assertAlmostEqual(deltaChar3, 0.0, 9)
        deltaChar4 = np.max(np.abs(funSysPar.Par[3] - systemParameters[1]))
        self.assertAlmostEqual(deltaChar4, 0.0, 9)
    def testNonEqDyn3(self):
        ...
        self.assertAlmostEqual(deltaChar1, 0.0, 8)
        deltaChar2 = np.max(np.abs(funSysPar.Par[1] - stateCoordinates[0]))
        self.assertAlmostEqual(deltaChar2, 0.0, 9)
        deltaChar3 = np.max(np.abs(funSysPar.Par[2] - reducedTemp[1]))
        self.assertAlmostEqual(deltaChar3, 0.0, 9)
