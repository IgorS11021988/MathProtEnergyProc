...
# Модульные тесты
class TestNonEqSystemComExpQ(unittest.TestCase):
    def setUp(self):
    ...
    def testNonEqDyn1(self):
        ...
        systemParameters1 = np.array([0.3, 0.5, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 200, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 615, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 21.9, 7.5, 52.6, 21.9, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 1.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1])  # Параметры системы
        ...
        systemParameters2 = np.array([0.3, 0.7, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 35, 75, 81, 210, 310, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 21.9, 2.5, 43.6, 21.9, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 1.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1])  # Параметры системы
        ...
        systemParameters3 = np.array([0.3, 0.7, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 65, 81, 210, 150, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 21.9, 3.5, 53.6, 21.7, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 2.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1])  # Параметры системы
        ...
        # Общая aункция эталонного результата
        ...
        (char1et1, char2et1) = (np.sum(Char1et1 * t1),
                                np.sum(Char2et1 * t1))
        ...
        (char1et2, char2et2) = (np.sum(Char1et2 * t2),
                                np.sum(Char2et2 * t2))
        ...
        (char1et3, char2et3) = (np.sum(Char1et3 * t3),
                                np.sum(Char2et3 * t3))
        ...
        Tints = np.array([Tint,
                          Tint])
        dyns = cDyn.ComputingExperimentQ(Tints,
                                         stateCoordinates,
                                         reducedTemp,
                                         systemParameters,
                                         t_evals=t_evals)
        (char1, char2, ind) = dyns

        # Эталонные индексы
        indsEt = np.arange(3).reshape(-1, 1)
    ...
    def testNonEqDyn2(self):
        ...
        # Моменты времени
        t_eval1 = np.linspace(0.0, Tint1, 10000)
        t_eval2 = np.linspace(0.0, Tint2, 5000)

        # Общая функция эталонного результата
        def countSystem1(t, allStParameters, systemParameters):
            # Выделение координат состояния и приведенных температур
            _stateCoordinates = allStParameters[0:-2]
            _reducedTemp = allStParameters[-2::]
        ...
        stateCoordinatesEt = integ.solve_ivp(countSystem1_1,
                                             (0.0, Tint1),
                                             np.hstack((stateCoordinates1, reducedTemp1)),  # Начальное состояние
                                             t_eval=t_eval1,  # Моменты времени
                                             atol=atol,
                                             rtol=rtol,
                                             method=method  # Метод численного интегрирования
                                             )  # Интегрирование дифференциальных уравнений
        ...
        stateCoordinatesEt = integ.solve_ivp(countSystem1_2,
                                             (0.0, Tint2),
                                             np.hstack((stateCoordinates2, reducedTemp1)),  # Начальное состояние
                                             t_eval=t_eval2,  # Моменты времени
                                             atol=atol,
                                             rtol=rtol,
                                             method=method  # Метод численного интегрирования
                                             )  # Интегрирование дифференциальных уравнений
        ...
        stateCoordinatesEt = integ.solve_ivp(countSystem1_3,
                                             (0.0, Tint2),
                                             np.hstack((stateCoordinates3, reducedTemp1)),  # Начальное состояние
                                             t_eval=t_eval2,  # Моменты времени
                                             atol=atol,
                                             rtol=rtol,
                                             method=method  # Метод численного интегрирования
        ...
        stateCoordinates = np.vstack([stateCoordinates1,
                                      stateCoordinates2,
                                      stateCoordinates3])
        reducedTemp = reducedTemp1.reshape(1, -1)
        systemParameters = np.vstack([systemParameters1,
                                      systemParameters2])
        t_evals = [t_eval1,
                   t_eval2,
    ...
    def testNonEqDyn3(self):
        ...
        # Рассчитываем эталонную динамику системы
        (atol, rtol, max_step) = funIntegAttributes(systemParameters1, method)
        stateCoordinatesEt = integ.solve_ivp(countSystem1_1,
                                             (0.0, Tint),
                                             np.hstack((stateCoordinates1, reducedTemp1)),  # Начальное состояние
                                             atol=atol,
        ...
        # Рассчитываем эталонную динамику системы
        (atol, rtol, max_step) = funIntegAttributes(systemParameters2, method)
        stateCoordinatesEt = integ.solve_ivp(countSystem1_2,
                                             (0.0, Tint),
                                             np.hstack((stateCoordinates2, reducedTemp2)),  # Начальное состояние
                                             atol=atol,
        ...
        # Рассчитываем эталонную динамику системы
        (atol, rtol, max_step) = funIntegAttributes(systemParameters3, method)
        stateCoordinatesEt = integ.solve_ivp(countSystem1_3,
                                             (0.0, Tint),
                                             np.hstack((stateCoordinates2, reducedTemp2)),  # Начальное состояние
                                             atol=atol,
    ...
    def testNonEqDyn4(self):
        ...
        systemParameters1 = systemParameters = np.array([0.3, 0.7, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 15.3, 44.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 21.9, 7.5, 53.6, 21.9, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 1.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1, 81.0, 5.13, 31.8, 3.3, 39.63, 54.3, 27.3, 0.15, 0.15])  # Параметры системы
        stateCoordinates2 = np.array([10, 20, 55, 75, 111, 213, 39, 45, 93, 81, 80, 64])  # Координаты состояния
        reducedTemp2 = np.array([350, 423])  # Приведенные температуры энергетических степеней свободы
        systemParameters2 = systemParameters = np.array([0.3, 0.7, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 36.9, 27.6, 4.53, 21.5, 7.5, 53.6, 21.9, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 1.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1, 81.0, 5.13, 31.8, 3.3, 39.63, 54.3, 27.3, 0.15, 0.15])  # Параметры системы
        ...
        systemParameters3 = systemParameters = np.array([0.3, 0.7, 10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 15.3, 41.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 21.9, 7.5, 53.6, 21.9, 45.3, 33.9, 159, 21.9, 42.5, 7.53, 31.5, 1.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1, 81.0, 5.13, 31.8, 3.3, 39.63, 54.3, 27.3, 0.15, 0.15])  # Параметры системы
        ...
        systemParameters4 = systemParameters = np.array([0.3, 0.7, 10, 20, 20, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 11.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5, 33.9, 27.3, 4.53, 20.9, 7.5, 53.6, 21.9, 45.3, 33.9, 159, 21.9, 43.5, 7.53, 37.5, 4.5, 120.3, 73.5, 15.3, 21.3, 0.15, 74.1, 81.0, 5.13, 31.8, 3.3, 39.63, 54.3, 27.3, 0.15, 0.15])  # Параметры системы
        ...
        # Функция эталонного результата 4
        ...
        (char1et1, char2et1, char3et1) = (np.sum(t1 * Char1et1 * Char3et1),
                                          np.sum(t1 * Char2et1 + Char3et1),
                                          np.sum(t1 * Char1et1))
        ...
        (char1et2, char2et2, char3et2) = (np.sum(t2 * Char1et2 * Char3et2),
                                          np.sum(t2 * Char2et2 + Char3et2),
                                          np.sum(t2 * Char1et2))
        ...
        (char1et3, char2et3, char3et3) = (np.sum(t3 * Char1et3 * Char3et3),
                                          np.sum(t3 * Char2et3 + Char3et3),
                                          np.sum(t3 * Char1et3))
        ...
        (char1et4, char2et4, char3et4) = (np.sum(t4 * Char1et4 * Char3et4),
                                          np.sum(t4 * Char2et4 + Char3et4),
                                          np.sum(t4 * Char1et4))
        ...
        def CompressFunction(dyn, index):
            # Выводим результат
            return (np.sum(dyn[0] * dyn[1] * dyn[3]),
                    np.sum(dyn[0] * dyn[2] + dyn[3]),
                    np.sum(dyn[0] * dyn[1]))
    ...
    def testNonEqDyn5(self):
        ...
        # Моменты времени
        t_eval1 = np.linspace(0.0, Tint1, 10000)
        t_eval2 = np.linspace(0.0, Tint2,  8000)
        t_eval3 = np.linspace(0.0, Tint3, 11000)
        ...
        stateCoordinatesEt = integ.solve_ivp(countSystem2_3,
                                             (0.0, Tint2),
                                             np.hstack((stateCoordinates2, reducedTemp3)),  # Начальное состояние
                                             t_eval=t_eval2,  # Моменты времени
                                             method=method  # Метод численного интегрирования
                                             )  # Интегрирование дифференциальных уравнений
        t = stateCoordinatesEt.t.reshape(-1, 1)  # Моменты времени
        ...
        stateCoordinatesEt = integ.solve_ivp(countSystem2_4,
                                             (0.0, Tint3),
                                             np.hstack((stateCoordinates2, reducedTemp3)),  # Начальное состояние
                                             t_eval=t_eval3,  # Моменты времени
                                             method=method  # Метод численного интегрирования
                                             )  # Интегрирование дифференциальных уравнений
        t = stateCoordinatesEt.t.reshape(-1, 1)  # Моменты времени
        stateCoordinatesEt = stateCoordinatesEt.y.transpose()  # Координаты состояния
        ...
        dyns = cDyn.ComputingExperimentQ(Tints,
                                         ...
                                         systemParameters,
                                         t_evals=t_evals)
        (char1, char2, char3, ind) = dyns

        # Эталонные индексы
        indsEt = np.arange(4).reshape(-1, 1)
    ...
    def testNonEqDyn6(self):
        ...
        reducedTemp = np.vstack([reducedTemp1,
                                 ...
                                 reducedTemp3,
                                 reducedTemp4])
        systemParameters = systemParameters1.reshape(1, -1)
        Tints = np.array([Tint,
                          Tint])
        dyns = cDyn.ComputingExperimentQ(Tints,
                                         stateCoordinates,
    ...
    def testNonEqDyn7(self):
        ...
        systemParameters1 = systemParameters = np.array([10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5])  # Параметры системы
        ...
        systemParameters2 = systemParameters = np.array([10, 20, 50, 70, 100, 200, 30, 45, 10, 25, 35, 55, 75, 81, 210, 300, 150, 153, 123, 15.3, 45.9, 5.63, 10, 6.5, 45.4, 315, 21.3, 4.5, 1.5])  # Параметры системы
    ...
    def testNonEqDyn8(self):
        ...
        # Моменты времени
        t_eval1 = np.linspace(0.0, Tint1, 10000)
        t_eval2 = np.linspace(0.0, Tint2, 12000)
    ...
    def testNonEqDyn9(self):
        ...
        # Эталонные индексы
        indsEt = np.arange(2).reshape(-1, 1)
